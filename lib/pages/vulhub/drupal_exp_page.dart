...
class DrupalExpPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(children: [
            const Icon(Icons.water_drop, color: AppColors.primary), const SizedBox(width: 8),
            Text('Drupal CVE-2018-7600 (Drupalgeddon2) Form API RCE',
                style: AppTextStyles.heading(size: 14, color: AppColors.primary)),
          ]),
        ),
        body: Padding(padding: const EdgeInsets.all(24),
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            vulhubInfoCard(Icons.water_drop, 'Drupal CVE-2018-7600', 'Drupalgeddon2 — Form API #post_render 回调 PHP 代码执行'),
            const SizedBox(height: 16),
            const Expanded(child: _DrupalCard()),
          ]),
        ),
      );
}

class _DrupalCard extends StatefulWidget {
  const _DrupalCard();
  @override
  State<_DrupalCard> createState() => _DrupalCardState();
}

class _DrupalCardState extends State<_DrupalCard> {
  ...
  DrupalExpService _svc() => DrupalExpService(baseUrl: _urlCtrl.text.trim(), timeout: Duration(seconds: int.tryParse(_timeoutCtrl.text.trim()) ?? 10));

  Future<void> _check() async {
    if (_urlCtrl.text.trim().isEmpty) { _log_('[!] 请输入目标 URL'); return; }
    setState(() => _running = true); _log_('[*] 检测 CVE-2018-7600 (Drupalgeddon2)...');
  ...
  Future<void> _exec() async {
    ...
    setState(() => _running = true); _log_('[*] RCE 执行: $cmd');
    try {
