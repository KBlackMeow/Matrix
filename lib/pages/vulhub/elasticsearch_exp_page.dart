...
class ElasticsearchExpPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(children: [
            const Icon(Icons.manage_search, color: AppColors.primary), const SizedBox(width: 8),
            Text('Elasticsearch CVE-2015-1427 Groovy 沙箱逃逸 RCE',
                style: AppTextStyles.heading(size: 14, color: AppColors.primary)),
          ]),
        ),
        body: Padding(padding: const EdgeInsets.all(24),
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            vulhubInfoCard(Icons.manage_search, 'Elasticsearch CVE-2015-1427', 'Groovy 脚本引擎沙箱逃逸，任意命令执行（< 1.3.8 / < 1.4.3）'),
            const SizedBox(height: 16),
            const Expanded(child: _ESCard()),
          ]),
        ),
      );
}

class _ESCard extends StatefulWidget {
  const _ESCard();
  @override
  State<_ESCard> createState() => _ESCardState();
}

class _ESCardState extends State<_ESCard> {
  ...
  ElasticsearchExpService _svc() => ElasticsearchExpService(baseUrl: _urlCtrl.text.trim(), timeout: Duration(seconds: int.tryParse(_timeoutCtrl.text.trim()) ?? 10));

  Future<void> _check() async {
    if (_urlCtrl.text.trim().isEmpty) { _log_('[!] 请输入目标 URL'); return; }
    setState(() => _running = true); _log_('[*] 检测 CVE-2015-1427 Groovy 沙箱逃逸...');
  ...
  Future<void> _exec() async {
    ...
    setState(() => _running = true); _log_('[*] Groovy 脚本 RCE: $cmd');
    try {
  ...
  Widget build(BuildContext context) => VulhubExpCardShell(
    ...
    leftPanel: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      vSecTitle('目标配置'),
      vTf(_urlCtrl, '目标 URL', 'http://target.com:9200/'),
