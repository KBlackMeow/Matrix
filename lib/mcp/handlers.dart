...
void registerAllTools(McpServer server, SessionPool pool, McpDatabase db,
  ...
  _registerFileWrite(server, pool, log);
  _registerFileUpload(server, pool, log);
  _registerFileDownload(server, pool, log);
  _registerFileDelete(server, pool, log);
...
void _registerShellList(McpServer server, McpDatabase db, McpActivityLogger? log) {
  server.registerTool(
    'shell_list',
    ...
    callback: _wrap('shell_list', (args, extra) async {
      try {
        ...
        final list = webshells.map((ws) => {
          ...
          'project_name': rows
              .firstWhere((r) => r['id'] == ws.id)['project_name'] ?? '',
...
void _registerShellAdd(McpServer server, McpDatabase db, McpActivityLogger? log) {
  server.registerTool(
    'shell_add',
    ...
    inputSchema: JsonSchema.object(
      properties: {
        ...
        'connector_type': JsonSchema.string(
          description: '连接器类型。PHP: php_eval, php_b64rot13, php_behinder, php_passthru；'
    ...
    callback: _wrap('shell_add', (args, extra) async {
      try {
        ...
        return _json({
          'id': ws.id, 'name': ws.name, 'url': ws.url,
          'type': ws.type, 'connector_type': ws.connectorType,
...
void _registerShellUse(McpServer server, SessionPool pool, McpDatabase db, McpActivityLogger? log) {
  server.registerTool(
    'shell_use',
    description: '设置默认 WebShell（后续操作可省略 shell_id 参数）',
    inputSchema: JsonSchema.object(
      properties: {
        'shell_id': JsonSchema.integer(description: 'WebShell ID'),
      },
      required: ['shell_id'],
    ),
...
void _registerShellRemove(McpServer server, SessionPool pool, McpDatabase db, McpActivityLogger? log) {
  server.registerTool(
    'shell_remove',
...
void _registerShellExec(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'shell_exec',
    description: '在目标机器上执行 shell 命令并返回输出',
    inputSchema: JsonSchema.object(
      properties: {
        'shell_id': JsonSchema.integer(description: 'WebShell ID（已设置默认 shell 时可省略）'),
...
void _registerFileList(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'file_list',
...
void _registerFileRead(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'file_read',
...
void _registerFileWrite(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'file_write',
...
void _registerFileUpload(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'file_upload',
    description: '上传本地文件到目标机器（自动判断文本/二进制路径）',
    inputSchema: JsonSchema.object(
      properties: {
        'shell_id': JsonSchema.integer(description: 'WebShell ID（已设置默认 shell 时可省略）'),
        'local_path': JsonSchema.string(description: '本机文件路径'),
    ...
    callback: _wrap('file_upload', (args, extra) async {
      try {
        ...
        if (!await file.exists()) return _error('本地文件不存在: $localPath');
        ...
        final useBinaryPath = svc.isWindowsTarget ||
            looksBinary ||
        ...
        if (!useBinaryPath) {
          ...
          ok = await svc.writeFile(remotePath, content);
        } else {
          ok = await svc.writeFileBinaryWithProgress(
            remotePath, bytes,
            (sent, total) {},
          );
        }
...
void _registerFileDownload(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'file_download',
    description: '从目标机器下载文件，返回 base64 编码的内容',
    inputSchema: JsonSchema.object(
      properties: {
        'shell_id': JsonSchema.integer(description: 'WebShell ID（已设置默认 shell 时可省略）'),
    ...
    callback: _wrap('file_download', (args, extra) async {
      try {
        final svc = await pool.resolve(_optShellId(args));
        final bytes = await svc.readFileBinary(args['remote_path'] as String);
        return _json({
...
void _registerFileDelete(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'file_delete',
    ...
    callback: _wrap('file_delete', (args, extra) async {
      try {
        final svc = await pool.resolve(_optShellId(args));
        final ok = await svc.deleteFile(args['path'] as String);
...
void _registerSystemInfo(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'system_info',
...
void _registerHomeDir(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'home_dir',
...
void _registerEnvList(McpServer server, SessionPool pool, McpActivityLogger? log) {
  server.registerTool(
    'env_list',
